...
SphGravityDriver::SphGravityDriver(Mesh *pm, ParameterInput *pin) {
  pmy_mesh_ = pm;
  // RemapColumns performs global all-to-all redistributions and assumes a fixed,
  // uniform block decomposition with exactly one MeshBlock owned by each MPI rank.
  // Check these constraints here so a bad production layout fails before timestepping.
  if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") != 0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in SphGravityDriver\n"
        << "The spherical-harmonic gravity solver requires "
  ...
  if (pmy_mesh_->multilevel || pmy_mesh_->adaptive) {
    std::stringstream msg;
    msg << "### FATAL ERROR in SphGravityDriver\n"
        << "The spherical-harmonic gravity solver does not support SMR or AMR.\n";
    ATHENA_ERROR(msg);
  }
  if (pmy_mesh_->nbtotal != Globals::nranks) {
    std::stringstream msg;
    msg << "### FATAL ERROR in SphGravityDriver\n"
        << "Exactly one MeshBlock per MPI rank is required. The mesh has "
        << pmy_mesh_->nbtotal << " blocks but the run has " << Globals::nranks
        << " MPI ranks.\n";
    ATHENA_ERROR(msg);
  }
  if (pmy_mesh_->mesh_size.nx1 < 5 || pmy_mesh_->mesh_size.nx2 < 5
      || pmy_mesh_->mesh_size.nx3 < 2) {
    std::stringstream msg;
    msg << "### FATAL ERROR in SphGravityDriver\n"
        << "Require nx1>=5, nx2>=5, and nx3>=2 for the fourth-order operators.\n";
    ATHENA_ERROR(msg);
  }
  if (pmy_mesh_->mesh_size.x3min != 0.0
      || pmy_mesh_->mesh_size.x3max != TWO_PI
      || pmy_mesh_->mesh_size.x3rat != 1.0) {
    std::stringstream msg;
    msg << "### FATAL ERROR in SphGravityDriver\n"
        << "Require a uniform, complete azimuthal domain x3=[0,2*pi].\n";
    ATHENA_ERROR(msg);
  }
