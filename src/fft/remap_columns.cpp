...
void RemapColumns::InitializeInd(){
  if (pmb->pmy_mesh->nbtotal != Globals::nranks) {
    std::stringstream msg;
    msg << "### FATAL ERROR in RemapColumns" << std::endl
        << "Global column remapping requires exactly one MeshBlock per MPI rank; got "
        << pmb->pmy_mesh->nbtotal << " blocks and " << Globals::nranks << " ranks."
        << std::endl;
    ATHENA_ERROR(msg);
  }
  ...
  if (!np_correct) {
    std::stringstream msg;
    msg << "### FATAL ERROR in RemapColumns" << std::endl
        << "The block decomposition cannot be remapped into complete coordinate "
        << "columns. Each block count p_i must factor into divisors of the two "
        << "other local MeshBlock dimensions." << std::endl
...
void RemapColumns::InitializeBoundary(AthenaArray<Real> * data)
{
  ...
  bvar = new CellCenteredBoundaryVariable(pmb, data, NULL, empty_flux);
