...
MeshBlock::MeshBlock(int igid, int ilid, LogicalLocation iloc, RegionSize input_block,
    ...
    hydro_theta_mask_enabled(false), hydro_theta_cut(0.0), hydro_theta_mask_bc(0),
    nuser_out_var(),
...
bool MeshBlock::IsHydroThetaMasked(int j) const {
  if (!hydro_theta_mask_enabled) return false;
  const Real theta = pcoord->x2v(j);
  return theta < hydro_theta_cut || theta > PI - hydro_theta_cut;
}
